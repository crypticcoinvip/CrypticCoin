...
This implementation of Crypticcoin is not resistant to side-channel attacks. You
should assume (even unprivileged) users who are running on the hardware, or who
are physically near the hardware, that your `crypticcoind` process is running on will
be able to:
...
unprivileged) on the hardware your `crypticcoind` process runs on until these
