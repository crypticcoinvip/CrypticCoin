...
Jack Grigg (22):
      ...
      Instruct users to run crypticcoin-fetch-params if network params aren't available
      ...
      Throw an error if crypticcoin.conf is missing
      Show a friendly message explaining why crypticcoind needs a crypticcoin.conf
      Fix gtest ordering broken by #1949
      Debian package lint
      Generate Debian control file to fix shlibs lint
      Create empty crypticcoin.conf during performance measurements
      Create empty crypticcoin.conf during coverage checks
...
Simon Liu (13):
      ...
      Remove UTF-8 BOM efbbbf from crypticcoin.conf to avoid problems with command line tools
      Closes #1097 so crypticcoin-cli now displays license info like crypticcoind.
