...
Alfie John (1):
      Rename libzerocash to libcrypticcoin
...
Nathan Wilcox (1):
      Fix a test name bug so that ``make cov-crypticcoin`` correctly runs the ``crypticcoin-gtest`` binary. Fixes #946.
...
Taylor Hornby (16):
      ...
      Turn on -Werror for the Crypticcoin build.
