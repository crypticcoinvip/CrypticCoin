...
Nathan Wilcox (4):
      ...
      Add a crypticcoin-gtest binary to our build with a single tautological test.
      Add coverage support scoped to only the crypticcoin-gtest run; invoke with make crypticcoin-cov; make cov is a superset.
...
Sean Bowe (49):
      ...
      Move incremental merkle tree tests to crypticcoin-gtest.
      ...
      Small nit fixes
      Run `crypticcoin-gtest` in `make check` and fix performance tests.
      ...
      Introduce new `libcrypticcoin` Crypticcoin protocol API and crypto constructions surrounding the zkSNARK circuit.
      ...
      Rename CRYPTICCOIN_ constants to ZC_.
...
Taylor Hornby (7):
      ...
      Fix RPC tests
      Rename bitcoin.conf and bitcoind.pid to crypticcoin.conf and crypticcoind.pid in qa/ and src/
