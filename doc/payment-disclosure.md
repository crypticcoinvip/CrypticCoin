...
This is an experimental feature.  Enable it by launching `crypticcoind` with flags:

    crypticcoind -experimentalfeatures -paymentdisclosure -debug=paymentdisclosure -txindex=1

These flags can also be set as options in `crypticcoin.conf`.
...
Alice the customer sends 10 CRYP to Bob the merchant at the shielded address shown on their website.  However, Bob is not sure if he received the funds.
