...
if [[ "$OSTYPE" == "darwin"* ]]; then
    PARAMS_DIR="$HOME/Library/Application Support/CrypticcoinParams"
else
    PARAMS_DIR="$HOME/.crypticcoin-params"
fi
...
Failed to fetch the Crypticcoin zkSNARK parameters!
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Crypticcoin zkSNARK parameters. Note that it is
