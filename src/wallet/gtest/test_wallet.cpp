...
CWalletTx GetValidReceive(const libcrypticcoin::SpendingKey& sk, CAmount value, bool randomInputs) {
    return GetValidReceive(*params, sk, value, randomInputs);
}

libcrypticcoin::Note GetNote(const libcrypticcoin::SpendingKey& sk,
                       const CTransaction& tx, size_t js, size_t n) {
    return GetNote(*params, sk, tx, js, n);
}

CWalletTx GetValidSpend(const libcrypticcoin::SpendingKey& sk,
...
TEST(wallet_tests, FindMyNotes) {
    ...
    auto sk = libcrypticcoin::SpendingKey::random();
    auto sk2 = libcrypticcoin::SpendingKey::random();
