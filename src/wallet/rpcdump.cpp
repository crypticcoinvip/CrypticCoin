...
UniValue importwallet_impl(const UniValue& params, bool fHelp, bool fImportZKeys)
{
    ...
    if (!file.is_open())
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Cannot open wallet dump file " + params[0].get_str());
    ...
    while (file.good()) {
        ...
        if (fImportZKeys) {
            try {
                ...
                // Not a valid spending key, so carry on and see if it's a Crypticcoin style address.
            }
        }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"crypticcoinaddress\"   (string, required) The Crypticcoin address for the private key\n"
            "\nResult:\n"
...
//    boost::filesystem::path exportfilepath = exportdir / clean;
    ...
    if (!file.is_open())
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Cannot open wallet dump file " + exportfilepath.string());
