...
double benchmark_verify_joinsplit(const JSDescription &joinsplit)
{
    ...
    uint256 joinSplitPubKey;
    auto verifier = libzcash::ProofVerifier::Strict();
    joinsplit.Verify(*pcrypticcoinParams, verifier, joinSplitPubKey);
    return timer_stop(tv_start);
}
...
double benchmark_try_decrypt_sprout_notes(size_t nKeys)
{
    ...
    auto sk = libzcash::SproutSpendingKey::random();
    auto tx = GetValidSproutReceive(*pcrypticcoinParams, sk, 10, true);
...
CWalletTx CreateSproutTxWithNoteData(const libzcash::SproutSpendingKey& sk) {
    auto wtx = GetValidSproutReceive(*pcrypticcoinParams, sk, 10, true);
    auto note = GetSproutNote(*pcrypticcoinParams, sk, wtx, 0, 1);
...
double benchmark_connectblock_slow()
{
    ...
    struct timeval tv_start;
    timer_start(tv_start);
    assert(ConnectBlock(block, state, &index, view, Params(), mnview, true));
