...
bool FakeMnTxChecker(CTransaction const &, Consensus::Params const &, int)
{
    return true;
}
...
TEST(Mempool, TxInputLimit) {
    ...
    EXPECT_FALSE(AcceptToMemoryPool(pool, state1, tx1, false, &missingInputs, FakeMnTxChecker));
    ...
    EXPECT_FALSE(AcceptToMemoryPool(pool, state2, tx1, false, &missingInputs, FakeMnTxChecker));
    ...
    EXPECT_FALSE(AcceptToMemoryPool(pool, state3, tx3, false, &missingInputs, FakeMnTxChecker));
    ...
    EXPECT_FALSE(AcceptToMemoryPool(pool, state4, tx3, false, &missingInputs, FakeMnTxChecker));
    ...
    EXPECT_FALSE(AcceptToMemoryPool(pool, state5, tx3, false, &missingInputs, FakeMnTxChecker));
    // The -mempooltxinputlimit check doesn't set a reason
    ...
    EXPECT_FALSE(AcceptToMemoryPool(pool, state6, tx3, false, &missingInputs, FakeMnTxChecker));
...
TEST(Mempool, SproutNegativeVersionTxWhenOverwinterActive) {
    ...
    // A Sprout transaction with version -3 is created using Sprout code (as found in Crypticcoind <= 1.0.14).
    ...
    // A Sprout transaction with version -3 created using Overwinter code (as found in Crypticcoind >= 1.0.15).
