...
bool CBlockTreeDB::ReadAddressUnspentIndex(uint160 addressHash, int type, std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue>>& unspentOutputs)
{
    ...
    while (pcursor->Valid()) {
        ...
        try {
            ...
            // ssKey >> indexKey;
            //std::pair<char, uint256> key;
            if (pcursor->GetKey(indexKey) && indexKey.type == DB_ADDRESSUNSPENTINDEX && indexKey.hashBytes == addressHash) {
...
bool CBlockTreeDB::ReadAddressIndex(uint160 addressHash, int type, std::vector<std::pair<CAddressIndexKey, CAmount>>& addressIndex, int start, int end)
{
    ...
    while (pcursor->Valid()) {
        ...
        try {
            ...
            CAddressIndexKey indexKey;
            if (pcursor->GetKey(indexKey) && indexKey.type == DB_ADDRESSINDEX && indexKey.hashBytes == addressHash) {
...
bool CBlockTreeDB::ReadTimestampIndex(const unsigned int& high, const unsigned int& low, const bool fActiveOnly, std::vector<std::pair<uint256, unsigned int>>& hashes)
{
    ...
    while (pcursor->Valid()) {
        ...
        try {
            ...
            CTimestampIndexKey indexKey;
            if (pcursor->GetKey(indexKey) && indexKey.timestamp < high) {
                if (fActiveOnly) {
