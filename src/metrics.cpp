        ...
        CAmount mature {0};
        {
            ...
            while (it != u->end()) {
                ...
                if (mapBlockIndex.count(hash) > 0 &&
                    ...
                    if ((height > 0) && (height <= consensusParams.GetLastFoundersRewardBlockHeight(height, Params().NetworkIDString() == "regtest"))) {
                        subsidy -= subsidy/5;
                    }
...
void ThreadShowMetricsScreen()
{
    ...
    if (isScreen) {
        ...
        // Thank you text
        std::cout << _("Thank you for running a Crypticcoin node!") << std::endl;
