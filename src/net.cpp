...
void CNode::Ban(const CSubNet& subNet, int64_t bantimeoffset, bool sinceUnixEpoch) {
    int64_t banTime = GetTime()+GetArg("-bantime", 10 * 60);  // Default 10-minutes ban
    if (bantimeoffset > 0)
...
bool BindListenPort(const CService &addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Crypticcoin is probably already running."), addrBind.ToString());
        else
...
void CNode::AskFor(const CInv& inv)
{
    ...
    // Each retry is 20 seconds after the last
