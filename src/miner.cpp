...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
    ...
    const std::vector<CTransaction> committedList = dpos::getController()->listCommittedTxs();
    {
        ...
        if ((nHeight > 0) && (nHeight <= chainparams.GetConsensus().GetLastFoundersRewardBlockHeight(nHeight, chainparams.NetworkIDString() == "regtest"))) {
        ...
        CValidationState state;
        if (!TestBlockValidity(state, chainparams, *pblock, pindexPrev, false, false, dvr))
...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    /// @todo Is it necessary? (merged from z2.0.4)
...
{
    LogPrintf("CrypticcoinMiner started\n");
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                if (GetArg("-mineraddress", "").empty()) {
                    LogPrintf("Error in CrypticcoinMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                } else {
                    // Should never reach here, because -mineraddress validity is checked in init.cpp
                    LogPrintf("Error in CrypticcoinMiner: Invalid -mineraddress\n");
                }
                return;
            }
    ...
    catch (const boost::thread_interrupted&)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("CrypticcoinMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("CrypticcoinMiner runtime error: %s\n", e.what());
        return;
    }
    miningTimer.stop();
    c.disconnect();
